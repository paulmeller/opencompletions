/**
 * Workspace file management for the OpenCompletions engine.
 *
 ...
 *
 * Ported from files.js (~730 lines).
 */
...
// ---------------------------------------------------------------------------
// State (survives Next.js HMR in dev via globalThis)
// ---------------------------------------------------------------------------

const globalForFiles = globalThis as typeof globalThis & {
  __ocWorkspaces?: Map<string, Workspace>;
};
if (!globalForFiles.__ocWorkspaces) {
  globalForFiles.__ocWorkspaces = new Map<string, Workspace>();
}
const workspaces = globalForFiles.__ocWorkspaces;
 ...
 */
export async function listFiles(workspaceId: string): Promise<FileEntry[]> {
  const ws = workspaces.get(workspaceId);
  if (!ws) throw Object.assign(new Error("Workspace not found"), { code: 404 });
  ws.lastAccessedAt = Date.now();
...
/**
 * Read a file from a workspace. Returns a Node.js Readable stream.
 */
...
export function getWorkspaceCwd(id: string): string | null {
  const ws = workspaces.get(id);
  if (!ws) return null;
  if (ws.backend === "local") return ws.localDir || null;
  return ws.remotePath || null;
}
 ...
 */
export async function cleanupExpired(ttlMs: number): Promise<void> {
  const now = Date.now();
 ...
 */
export function cleanupOrphaned(): void {
  try {
    const tmpDir = os.tmpdir();
    const entries = fs.readdirSync(tmpDir);
    const knownLocalDirs = new Set<string>();
...
export function buildFileManifest(workspaceId: string): string | null {
  const ws = workspaces.get(workspaceId);
 ...
 */
export function validateFilename(name: string | null | undefined): string | null {
 ...
 */
export function createTarGz(tarFiles: Array<{ name: string; buffer: Buffer }>): Buffer {
  const blocks: Buffer[] = [];
...
async function spriteWriteFile(
  ...
  const url = `${config.spriteApi}/v1/sprites/${encodeURIComponent(spriteName)}/fs/write?path=${encodedPath}&workingDir=/home/sprite&mkdir=true`;
...
async function spriteReadFile(
  ...
  const config = getConfig();
  const encodedPath = encodeURIComponent(remotePath);
  const url = `${config.spriteApi}/v1/sprites/${encodeURIComponent(spriteName)}/fs/read?path=${encodedPath}`;
...
async function spriteListDir(
  spriteName: string,
  remotePath: string,
): Promise<DirEntry[]> {
  const config = getConfig();
  const encodedPath = encodeURIComponent(remotePath);
  const url = `${config.spriteApi}/v1/sprites/${encodeURIComponent(spriteName)}/fs/list?path=${encodedPath}&workingDir=/home/sprite`;
  ...
  return ((data.entries || data || []) as DirEntry[]).filter((e) => e.type === "file");
}

async function spriteDeleteDir(
  spriteName: string,
  remotePath: string,
): Promise<void> {
  const config = getConfig();
  const encodedPath = encodeURIComponent(remotePath);
  const url = `${config.spriteApi}/v1/sprites/${encodeURIComponent(spriteName)}/fs/delete?path=${encodedPath}`;
 ...
 * Uses GET instead of HEAD because Sprites API doesn't support HEAD on /fs/read.
 */
async function spriteWorkspaceExists(
  spriteName: string,
  remotePath: string,
): Promise<boolean> {
  const config = getConfig();
...
function vercelUrl(urlPath: string): string {
  ...
  return `https://api.vercel.com${urlPath}${sep}teamId=${encodeURIComponent(config.vercelTeamId)}`;
}

function vercelHdrs(): Record<string, string> {
  const config = getConfig();
  return {
...
async function vercelMkdir(sandboxId: string, dirPath: string): Promise<void> {
...
async function vercelListDir(
  ...
  for (const line of text.split("\n")) {
    ...
    try {
      const event = JSON.parse(line) as { stream?: string; data?: string };
...
async function vercelDeleteDir(
  sandboxId: string,
  dirPath: string,
): Promise<void> {
 ...
 */
async function vercelWorkspaceExists(
  sandboxId: string,
  dirPath: string,
): Promise<boolean> {
  const config = getConfig();
 ...
 */
export async function validateWorkspaceExists(workspaceId: string): Promise<boolean> {
  const ws = workspaces.get(workspaceId);
  if (!ws) return false;
